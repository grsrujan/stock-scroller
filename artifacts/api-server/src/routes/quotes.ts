...
async function fetchFinancialsFor(symbol: string): Promise<void> {
  try {
    ...
    const fd = (summary as { financialData?: { totalRevenue?: number } } | null)
      ?.financialData;
    const ks = (
      summary as { defaultKeyStatistics?: { netIncomeToCommon?: number } } | null
    )?.defaultKeyStatistics;
    const revenue =
      typeof fd?.totalRevenue === "number" ? fd.totalRevenue : null;
    const profit =
      typeof ks?.netIncomeToCommon === "number" ? ks.netIncomeToCommon : null;
...
function getFinancials(sym: string): Financials {
  return (
    financialsCache.get(sym)?.fin ?? { revenue: null, profit: null }
  );
}
  ...
  try {
    ...
    const quotes: CachedQuote[] = symbols.map((sym) => {
      ...
      const changePercent =
        ...
        null;
      const trailingDivYield = q.trailingAnnualDividendYield as
        | number
        | undefined;
      const dividendYieldPct =
        typeof trailingDivYield === "number" ? trailingDivYield * 100 : 0;
      return {
        symbol: sym,
        price,
        change,
        changePercent,
        fiftyTwoHigh:
          (q.fiftyTwoWeekHigh as number | undefined) ?? null,
        fiftyTwoLow:
          (q.fiftyTwoWeekLow as number | undefined) ?? null,
        dividendYieldPct,
    ...
    req.log.error({ err }, "Failed to fetch Yahoo quotes");
    if (cached) {
